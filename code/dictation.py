...
# @mod.capture(rule="({user.vocabulary} | {user.punctuation} | {user.prose_snippets} |  <user.number_prefix> | <user.percent_suffix> | <user.abbreviation> | <phrase> |  <user.prose_number> | <user.prose_modifier>)+")
@mod.capture(
    rule=
    "({user.vocabulary} | {user.punctuation} | {user.prose_snippets} | <phrase> | <user.prose_number> | <user.prose_modifier>)+ | <user.percent_suffix> | <user.abbreviation> "
)
def prose(m) -> str:
...
def apply_formatting(m):
    ...
    for item in m:
        ...
        if isinstance(item, Callable):
            ...
            words = (actions.dictate.replace_words(
                actions.dictate.parse_words(item)) if isinstance(
                    item, grammar.vm.Phrase) else [item])
            for word in words:
...
class Actions:
    ...
    def dictation_reformat_no_cap():
        ...
        reformat_last_utterance(
            lambda s: format_first_letter(s, lambda c: c.lower()))
